...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_payment'])) {
    ...
    try {
        ...
        if ($action === 'verify') {
            ...
            // Check stock availability again before deduction
            $stock_errors = [];
            foreach ($order_items as $item) {
                ...
                if ($stock_row = $stock_result->fetch_assoc()) {
                    if ($stock_row['display_quantity'] < $item['quantity']) {
                        $stock_errors[] = "Product '{$item['product_name']}' only has {$stock_row['display_quantity']} available, but order requires {$item['quantity']}";
                    }
                } else {
            ...
            foreach ($order_items as $item) {
                ...
                // Log transaction
                $notes = "Customer purchase - Order #{$order_id} - Payment verified by admin";
            ...
            $message = "Payment verified successfully! Stock has been deducted and order status updated to 'confirmed'.";
                            ...
                            <form method="POST">
                                ...
                                <div class="verification-actions">
                                    <button type="submit" name="action" value="verify" class="btn-verify">
                                        <i class='bx bx-check'></i> Verify Payment & Deduct Stock
                                    </button>
