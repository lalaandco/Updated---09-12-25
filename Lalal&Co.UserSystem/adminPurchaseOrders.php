    ...
    if (isset($_POST['action'])) {
        ...
        try {
            ...
            if ($_POST['action'] === 'create_purchase') {
                ...
                // ✅ FIX: Add to BOTH warehouse AND display inventory (auto-sync)
                foreach ($items as $item) {
                    ...
                    // ✅ Update BOTH inventory_quantity AND display_quantity
                    ...
                    $display_after = $display_before + $item['quantity'];
                    $notes = "Received from purchase order #{$purchase_id} - auto-synced to warehouse and display";
                    $admin_email = $_SESSION['admin_email'] ?? 'admin';
                    ...
                    $stmt->bind_param("iiiiiissi", $item['product_id'], $item['quantity'], $inventory_before, $inventory_after, $display_before, $display_after, $notes, $admin_email, $purchase_id);
                    $stmt->execute();
                }
                ...
                $conn->commit();
                $_SESSION['success_message'] = "Purchase order received! Stock added to both warehouse and display automatically.";
                header('Location: adminPurchaseOrders.php');
                exit();
            }
...
<body>
    ...
    <div id="createModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" onsubmit="return submitPurchaseOrder(event)">
                ...
                <div class="form-group">
                    <label>Notes</label>
                    <textarea name="notes" rows="3" placeholder="Stock will be automatically added to both warehouse and display when received..."></textarea>
                </div>
    ...
    <script>
        ...
        function openCreateModal() {
            ...
            // Check if coming from inventory page with pre-selected product
            const urlParams = new URLSearchParams(window.location.search);
            const productId = urlParams.get('product_id') || sessionStorage.getItem('preselected_product_id');
            
            if (productId) {
                // Auto-select the product in the first row
                const firstSelect = document.querySelector('.product-select');
                if (firstSelect) {
                    firstSelect.value = productId;
                    
                    // Clear session storage
                    sessionStorage.removeItem('preselected_product_id');
                    sessionStorage.removeItem('preselected_product_name');
        ...
        function receivePurchase(purchaseId) {
            if (confirm('Mark this purchase order as received? Stock will be automatically added to both warehouse AND display.')) {
