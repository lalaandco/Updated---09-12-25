...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    try {
        ...
        // Get selected items from POST (sent from JavaScript)
        ...
        foreach ($selected_items as $item) {
            ...
            // Lock the row for update and check DISPLAY quantity
            $stock_stmt = $conn->prepare("SELECT display_quantity, product_name, inventory_quantity FROM product_tbl WHERE product_id = ? FOR UPDATE");
            ...
            if ($stock_row = $stock_result->fetch_assoc()) {
                ...
                if ($available_display < $requested_qty) {
                    $stock_errors[] = [
                        ...
                        'warehouse_stock' => $available_inventory
                    ];
                }
            } else {
                $stock_errors[] = [
        ...
        // If there are stock errors, rollback and show error
            ...
            foreach ($stock_errors as $error) {
                if (isset($error['available'])) {
                    if ($error['available'] == 0) {
                        if ($error['warehouse_stock'] > 0) {
                            $error_message .= "• <strong>{$error['product_name']}</strong>: Out of stock in display (Warehouse has {$error['warehouse_stock']} units - contact admin to restock)<br>";
                        } else {
                            $error_message .= "• <strong>{$error['product_name']}</strong>: Out of stock (You requested {$error['requested']})<br>";
                        }
                    } else {
                        $error_message .= "• <strong>{$error['product_name']}</strong>: Only {$error['available']} available for purchase (You requested {$error['requested']})<br>";
                    }
                } else {
        ...
        // When inserting order items, DECREASE DISPLAY QUANTITY ONLY
        ...
        foreach ($selected_items as $item) {
            ...
            // DECREASE DISPLAY QUANTITY ONLY (inventory stays the same)
            ...
            $transaction_stmt->bind_param("iiiiis", $product_id, $quantity, $inv_before, $inv_before, $display_before, $display_after, $notes);
        ...
        if (!empty($stock_errors)) {
            ...
            // Redirect back to checkout to display error
            header("Location: checkout.php");
            exit();
        }
        ...
        $item_stmt = $conn->prepare("INSERT INTO order_items (order_id, product_id, product_name, product_price, quantity, subtotal) VALUES (?, ?, ?, ?, ?, ?)");
        $stock_update_stmt = $conn->prepare("UPDATE product_tbl SET product_quantity = product_quantity - ? WHERE product_id = ?");
        ...
        foreach ($selected_items as $item) {
            ...
            // Remove purchased items from cart session
            if (isset($_SESSION['cart'][$product_id])) {
                unset($_SESSION['cart'][$product_id]);
            }
        }
            ...
            <form method="POST" id="checkout-form">
                ...
                <div class="payment-methods">
                    <div class="payment-option" onclick="selectPayment('gcash')">
                        ...
                        <div class="payment-icon"><img src="https://gadgetsmagazine.com.ph/wp-content/uploads/2020/05/GCASH-logo.jpg" alt="Gcash" class="payment-icon"></div>
                        <div>GCash</div>
                    </div>
