...
";
    ...
    foreach ($online_sales as $sale) {
        ...
        $combined_sales[$date]['cancelled_orders'] = $sale['cancelled_orders'] ?? 0;
        $combined_sales[$date]['refunded_orders'] = $sale['refunded_orders'] ?? 0;
        $combined_sales[$date]['cancelled_amount'] = $sale['cancelled_amount'] ?? 0;
        $combined_sales[$date]['refunded_amount'] = $sale['refunded_amount'] ?? 0;
    }
    ...
    $cancelled_orders_count = $conn->query("SELECT COUNT(DISTINCT order_id) FROM orders WHERE DATE(order_date) BETWEEN '{$date_from}' AND '{$date_to}' AND status = 'cancelled'")->fetch_row()[0];
    $refunded_orders_count = $conn->query("SELECT COUNT(DISTINCT order_id) FROM orders WHERE DATE(order_date) BETWEEN '{$date_from}' AND '{$date_to}' AND status = 'refunded'")->fetch_row()[0];
    $cancelled_revenue = $conn->query("SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE DATE(order_date) BETWEEN '{$date_from}' AND '{$date_to}' AND status = 'cancelled'")->fetch_row()[0];        $refunded_revenue = $conn->query("SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE DATE(order_date) BETWEEN '{$date_from}' AND '{$date_to}' AND status = 'refunded'")->fetch_row()[0];
    ...
    $summary = [
        ...
        // ✅ NEW: Cancelled and refunded stats
        'cancelled_orders' => $cancelled_orders_count,
        'refunded_orders' => $refunded_orders_count,
        'cancelled_amount' => $cancelled_revenue,
        'refunded_amount' => $refunded_revenue,
        'net_revenue' => ($online_total_revenue + $walkin_revenue) - $refunded_revenue
    ];

    // ✅ NEW: Get cancelled and refunded order stats
}
...
if ($report_type === 'stock') {
    $stock_query = "
        SELECT 
            ...
            p.display_quantity,
            p.product_price,
            -- Use display_quantity as the ACTUAL stock count (what customers can buy)
            p.display_quantity * p.product_price as stock_value
        FROM product_tbl p
        ORDER BY p.display_quantity ASC
    ";
    
    $stock_data = $conn->query($stock_query)->fetch_all(MYSQLI_ASSOC);
    
    // Stock summary by category - Use display_quantity only
    $category_stock_query = "
        SELECT 
            category,
            COUNT(product_id) as product_count,
            SUM(display_quantity) as total_stock,
            SUM(display_quantity * product_price) as total_value
        FROM product_tbl
        GROUP BY category
    ";
    ...
    // Low stock alerts - Use display_quantity
    $low_stock_query = "
        SELECT 
            product_name,
            inventory_quantity,
        ...
        <?php if ($report_type === 'sales'): ?>
            <div class="summary-cards">
                ...
                <div class="summary-card">
                    ...
                    <div class="subtext">
                        Lost: ₱<?php echo number_format($summary['cancelled_amount'] ?? 0, 2); ?>
                    </div>
                </div>
                ...
                <div class="summary-card" >
                    ...
                    <div class="subtext">
                        Refunded: ₱<?php echo number_format($summary['refunded_amount'] ?? 0, 2); ?>
                    </div>
                </div>
            ...
            <div class="data-table">
                ...
                <table>
                    <thead>
                        <tr>
                            <th>Product</th>
                            <th>Stock-in (Storage)</th>
                            <th>Display (Available)</th>
                            <th>Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($item['display_quantity'] == 0): ?>
                                    <span class="status-badge badge-low">Out of Stock</span>
                                <?php elseif ($item['display_quantity'] < 10): ?>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>₱<?php echo number_format($stock['product_price'], 2); ?></td>
