...
$pending_cancellations_query = "
    SELECT COUNT(*) as pending_count
    FROM order_cancellations
    WHERE status = 'pending'
";
$pending_cancellations_result = $conn->query($pending_cancellations_query);
$pending_cancellations = $pending_cancellations_result ? $pending_cancellations_result->fetch_assoc()['pending_count'] : 0;

// ✅ NEW: Get pending refunds count
...
$pending_refunds = $pending_refunds_result ? $pending_refunds_result->fetch_assoc()['pending_count'] : 0;
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="card cancelled-card">
            <a href="#cancellations" onclick="showCancellations(); return false;">
                <i class='bx bx-x-circle'></i>
                <h4>Cancelled Orders</h4>
                <?php if ($pending_cancellations > 0): ?>
                    <span class="stat-badge badge-danger"><?php echo $pending_cancellations; ?> pending</span>
                <?php endif; ?>
            </a>
        </div>
        
        <!-- ✅ NEW: Refund Requests Card -->
        <div class="card refund-card">
            <a href="adminPaymentVerification.php?tab=refunds">
                <i class='bx bx-undo'></i>
                <h4>Refund Requests</h4>
                <?php if ($pending_refunds > 0): ?>
                    <span class="stat-badge badge-refund"><?php echo $pending_refunds; ?> awaiting</span>
                <?php endif; ?>
            </a>
        </div>
    ...
    <script>
// ✅ NEW: Show cancellations modal/section
function showCancellations() {
    // Fetch and display pending cancellations
    fetch('get_pending_cancellations.php')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                displayCancellationsModal(data.cancellations);
            } else {
                alert('No pending cancellations');
            }
        })
        .catch(error => {
...
function displayCancellationsModal(cancellations) {
    let html = `
        <div class="modal-overlay" id="cancellationsModal" style="display: flex;">
            <div class="modal-content" style="max-width: 900px; max-height: 90vh; overflow-y: auto;">
                <div class="modal-header">
                    <h2><i class='bx bx-x-circle'></i> Pending Cancellation Requests</h2>
                    <span class="close" onclick="closeCancellationsModal()">&times;</span>
                </div>
                <div class="modal-body">
    `;
    
    if (cancellations.length === 0) {
        html += '<p style="text-align: center; padding: 40px; color: #999;">No pending cancellations</p>';
    } else {
        cancellations.forEach(cancel => {
            html += `
                <div class="cancellation-item" style="background: white; padding: 20px; margin-bottom: 15px; border-radius: 8px; border: 1px solid #ddd;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
                        <div>
                            <strong>Order #${String(cancel.order_id).padStart(8, '0')}</strong>
                            <p style="margin: 5px 0; color: #666;">Customer: ${cancel.full_name}</p>
                            <p style="margin: 5px 0; color: #666;">Email: ${cancel.user_email}</p>
                            <p style="margin: 5px 0; color: #666;">Phone: ${cancel.phone || 'N/A'}</p>
                        </div>
                        <div style="text-align: right;">
                            <div style="background: #fff3cd; color: #856404; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">
                                ₱${parseFloat(cancel.total_amount).toFixed(2)}
                            </div>
                            <p style="margin: 10px 0 0 0; font-size: 12px; color: #999;">
                                ${new Date(cancel.created_at).toLocaleDateString()}
                            </p>
                        </div>
                    </div>
                    ...
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 6px; margin-bottom: 15px;">
                        ...
                        <p style="margin: 8px 0 0 0;">${cancel.reason}</p>
                    </div>
                    
                    <div style="display: flex; gap: 10px;">
                        <button onclick="processCancellation(${cancel.cancellation_id}, 'approve')" 
                                style="flex: 1; padding: 10px; background: #4CAF50; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600;">
                            ✓ Approve Cancellation
                        </button>
                        <button onclick="processCancellation(${cancel.cancellation_id}, 'reject')" 
                                style="flex: 1; padding: 10px; background: #f44336; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600;">
                            ✗ Reject Request
                        </button>
                    </div>
                </div>
...
function closeCancellationsModal() {
    const modal = document.getElementById('cancellationsModal');
    if (modal) {
        modal.remove();
    }
}
