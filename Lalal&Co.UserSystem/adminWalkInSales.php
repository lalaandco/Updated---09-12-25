...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        // ✅ Verify BOTH warehouse AND display stock availability
        foreach ($products as $item) {
            ...
            $stock_stmt = $conn->prepare("SELECT inventory_quantity, display_quantity, product_name FROM product_tbl WHERE product_id = ? FOR UPDATE");
            ...
            // Check BOTH warehouse and display have enough stock
            ...
            if ($stock_row['display_quantity'] < $quantity) {
                throw new Exception("Insufficient display stock for {$stock_row['product_name']}. Available: {$stock_row['display_quantity']}");
            }
        ...
        // ✅ Process each product - DECREASE BOTH WAREHOUSE AND DISPLAY
        foreach ($products as $item) {
            $product_id = intval($item['product_id']);
            $quantity = intval($item['quantity']);
            ...
            // Get current quantities before update
            $qty_stmt = $conn->prepare("SELECT inventory_quantity, display_quantity FROM product_tbl WHERE product_id = ?");
            ...
            $display_before = $qty_row['display_quantity'];
            $warehouse_after = $warehouse_before - $quantity;
            $display_after = $display_before - $quantity;
            
            // ✅ DECREASE BOTH WAREHOUSE AND DISPLAY for walk-in sales
            $update_stmt = $conn->prepare("UPDATE product_tbl SET inventory_quantity = inventory_quantity - ?, display_quantity = display_quantity - ? WHERE product_id = ?");
            $update_stmt->bind_param("iii", $quantity, $quantity, $product_id);
            ...
            // Log transaction - both warehouse and display decreased
            ...
            $notes = "Walk-in purchase - Sale #{$sale_id} - Product sold from physical store (both warehouse and display deducted)";
            $trans_stmt->bind_param("iiiiiss", $product_id, $negative_qty, $warehouse_before, $warehouse_after, $display_before, $display_after, $notes);
            $trans_stmt->execute();
            $trans_stmt->close();
        }
            ...
            <div class="product-list" id="product-list">
                <?php foreach ($all_products as $product): ?>
                    <button type="button" class="product-btn" 
                            data-category="<?php echo $product['display_quantity'] > 0 ? 'in-stock' : 'out-of-stock'; ?>" 
                            data-product-category="<?php echo htmlspecialchars($product['category'] ?? 'Others'); ?>" 
                        ...
                        <div class="product-stock">
                            Warehouse: <?php echo $product['inventory_quantity']; ?> | 
                            Display: <?php echo $product['display_quantity']; ?>
                        </div>
                    </button>
                <?php endforeach; ?>
            </div>
            ...
            <div class="payment-section">
                ...
                <form id="saleForm" method="POST" onsubmit="submitSale(event)">
                    ...
                    <p style="font-size: 11px; color: #666; margin-top: 10px; text-align: center;">
                        ⚠️ This will deduct from both warehouse and display stock
                    </p>
                </form>
            </div>
        </div>
...
function addToCart(product) {
    ...
    // Use the MINIMUM of warehouse and display as the max available
    const maxAvailable = Math.min(product.inventory_quantity, product.display_quantity);
    ...
    const qty = prompt(`Add ${product.product_name}\nMax available: ${maxAvailable}\n(Limited by both warehouse and display stock):`, '1');
    
    if (qty === null) return;
    
    const quantity = parseInt(qty);
    if (isNaN(quantity) || quantity <= 0 || quantity > maxAvailable) {
        alert(`Invalid quantity. Maximum available: ${maxAvailable}`);
        return;
    }

    const existing = cart.find(item => item.product_id === product.product_id);
    if (existing) {
        if (existing.quantity + quantity > maxAvailable) {
            alert(`Cannot add more. Maximum available: ${maxAvailable}`);
            return;
        }
...
function updateQty(productId, newQty) {
    ...
    if (item) {
        ...
        if (qty > item.max_available) {
            alert(`Maximum available quantity is ${item.max_available}`);
            renderCart();
            return;
        }
